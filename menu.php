...
while($fila=$res->fetch(PDO::FETCH_ASSOC)){
    ...
    while($fila1=$ressubblk->fetch(PDO::FETCH_ASSOC)){
        if($fila1[id_url]===null){
                    ...
                    echo '<ul class="dropdown">';
                        //URLS
                        $resurl=$conex->query("select b.*,c.nivel from "
                                . "menu_urls a "
                                . "join urls b "
                                . "on a.id_url=b.id "
                                . "join menu_permisos c "
                                . "on c.id_url=b.id "
                                . "join contactos d "
                                . "on d.id=c.id_usuario "
                                . "where a.id_subloque=$fila1[id] and a.id_bloque=$fila[id] and c.nivel<>0 and d.id=$_SESSION[id_usuario]");
                    ...
                    echo "</li>" ;          
                                ...
                                $url=$conex->query("select * from urls a  join menu_permisos b on a.id=b.id_url join contactos c on c.id=b.id_usuario  where a.id=$fila1[id_url] and b.nivel<>0 and c.id=$_SESSION[id_usuario]")->fetch();
